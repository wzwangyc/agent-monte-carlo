...
Financial markets are complex adaptive systems where \textit{fat tails}, \textit{volatility clustering}, and \textit{endogenous crashes} emerge from interactions between heterogeneous agents. These stylized facts---fat tails (kurtosis $\approx 19$ vs 3 for normal), volatility clustering (ACF $\approx 0.21$), and crashes ($\approx 3\%$ per year)---pose fundamental challenges to traditional models based on geometric Brownian motion.\footnote{See \citet{cont2001empirical} for a comprehensive survey.}

This paper establishes the theoretical foundations and empirical validation of \textbf{Agent Monte Carlo} (Agent MC), a simulation framework that reproduces these stylized facts with rigorous proofs, comprehensive calibration, and policy relevance.
