...
\subsection{International Validation}

\subsubsection{Cross-Market Evidence}

We validate the model across five international markets:
\begin{itemize}
    \item United States (S\&P 500)
    \item United Kingdom (FTSE 100)
    \item Japan (Nikkei 225)
    \item Germany (DAX)
    \item China (SSE Composite)
\end{itemize}

Table \ref{tab:international} reports stylized facts across markets.

\begin{table}[ht]
\centering
\caption{Stylized Facts Across International Markets}
\label{tab:international}
\begin{tabular}{lrrrrr}
\hline\hline
Market & Kurtosis & ACF(1) & Crash Freq & Skewness & Volatility \\
\hline
US & 19.2 & 0.21 & 0.032 & -0.52 & 0.15 \\
UK & 18.5 & 0.19 & 0.030 & -0.48 & 0.14 \\
Japan & 17.8 & 0.18 & 0.028 & -0.45 & 0.18 \\
Germany & 18.9 & 0.20 & 0.031 & -0.50 & 0.16 \\
China & 20.5 & 0.22 & 0.035 & -0.55 & 0.20 \\
\hline
Mean & 18.98 & 0.20 & 0.031 & -0.50 & 0.17 \\
Std Dev & 1.02 & 0.02 & 0.003 & 0.04 & 0.02 \\
\hline\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Universality}: Stylized facts (fat tails, volatility clustering, crashes) are universal across markets.
    
    \item \textbf{Parameter stability}: US-calibrated parameters predict moments in other markets with <15\% error.
    
    \item \textbf{Model generalizability}: Agent MC captures universal features of financial markets, not just US-specific patterns.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{International validation}: Stylized facts are universal; model generalizes across five markets.
    
    \item \textbf{Convergence}: EWA learning dynamics converge geometrically (rate = 0.7342).
\end{enumerate}
