...
Table \ref{tab:calibrated_params} reports the calibrated parameter values with bootstrap standard errors (1,000 replications).

\begin{table}[ht]
\centering
\caption{Calibrated Parameter Values with Bootstrap Standard Errors}
\label{tab:calibrated_params}
\begin{tabular}{lrrrr}
\hline\hline
Parameter & Estimate & Std Error & 95\% CI & Benchmark \\
\hline
Herding strength & 0.72 & 0.08 & [0.58, 0.85] & 0.50 \\
Learning speed ($\phi$) & 0.85 & 0.03 & [0.80, 0.90] & 0.88 \\
Imagination ($\delta$) & 0.55 & 0.05 & [0.46, 0.65] & 0.58 \\
Experience growth ($\rho$) & 0.82 & 0.04 & [0.75, 0.89] & 0.85 \\
Selection ($\lambda$) & 1.8 & 0.3 & [1.3, 2.4] & 1.5 \\
Informed proportion ($\mu$) & 0.15 & 0.04 & [0.08, 0.23] & 0.10 \\
Noise volatility & 0.18 & 0.03 & [0.13, 0.24] & 0.15 \\
\hline\hline
\end{tabular}
\end{table}

\textbf{Note:} Bootstrap standard errors computed using 1,000 replications. 95\% confidence intervals using percentile method. All calibrated parameters are within reasonable ranges of benchmark values from the literature.
