...
# Page configuration
st.set_page_config(
    page_title="Agent Monte Carlo ü¶Å",
    page_icon="ü¶Å",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Custom CSS forÊûÅËá¥ styling
st.markdown("""
<style>
    .main-header {
        font-size: 3.5rem;
...
def main():
    ...
    st.markdown('<h1 class="main-header">ü¶Å Agent Monte Carlo</h1>', unsafe_allow_html=True)
    ...
    # Mode selection
    st.sidebar.markdown("### üîë Access Mode")
    ...
    if st.session_state.is_pro_mode:
        st.sidebar.markdown('<div class="pro-tier-badge">üöÄ PRO MODE</div>', unsafe_allow_html=True)
        ...
        if api_key:
            ...
            st.sidebar.success("‚ú?API key saved!")
        else:
            st.sidebar.warning("‚ö†Ô∏è Please enter API key for Pro Mode")
        
        st.sidebar.markdown("""
        **Pro Mode Benefits:**
        - ‚ú?Unlimited simulations
        - ‚ú?Custom data sources
        ...
        st.sidebar.markdown('<div class="free-tier-badge">üÜì FREE TIER</div>', unsafe_allow_html=True)
        ...
        if can_proceed:
            st.sidebar.success(f"‚ú?{remaining} attempts remaining today")
        else:
            st.sidebar.error("‚ù?Daily limit reached. Try again tomorrow or switch to Pro Mode!")
        
        st.sidebar.markdown("""
        **Free Tier Includes:**
        - ‚ú?10 simulations/day
        - ‚ú?Full MC comparison
    ...
    st.markdown("### üéõÔ∏?Simulation Parameters")
    ...
    # Run button
    run_button = st.button("üöÄ Run Dual Simulation", type="primary", use_container_width=True)
    
    if run_button:
        can_proceed, remaining = check_rate_limit()
        
        if st.session_state.is_pro_mode and not st.session_state.api_key:
            st.error("‚ù?Please enter your API key first!")
        elif not can_proceed and not st.session_state.is_pro_mode:
            st.error("‚ù?Daily limit reached! Switch to Pro Mode for unlimited simulations.")
        else:
            # Progress bar
            ...
            # Run Traditional MC
            status_text.text("üîÑ Running Traditional Monte Carlo (GBM)...")
            ...
            # Run Agent MC
            status_text.text("ü§ñ Running Agent-Based Monte Carlo...")
            ...
            status_text.text("‚ú?Simulation complete!")
            
            st.success(
                f"‚ú?Simulation completed! " 
            ...
            st.markdown("---")
            st.markdown("### üìà Full Visualization Comparison")
            ...
            with col1:
                st.markdown("### üéØ Risk Profile Comparison")
                radar_chart = create_risk_metrics_comparison(trad_metrics, agent_metrics)
                st.plotly_chart(radar_chart, use_container_width=True)
            
            with col2:
                st.markdown("### üìä Volatility Clustering (Agent MC)")
    ...
    <div style="text-align: center; color: #666; padding: 2rem;">
        <p><strong>ü¶Å Agent Monte Carlo</strong> | Enterprise-Grade Risk Simulation</p>
